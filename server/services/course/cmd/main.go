...
func main() {
	...
	logger.Init(cfg.Env)
	log := logger.Get()
	log.Info().Msg("Starting Course Service...")
	...
	handler := course.NewHandler(repo)
	...
	coursepb.RegisterCourseServiceServer(grpcServer, handler)
	reflection.Register(grpcServer)

	go func() {
		log.Info().Str("port", cfg.Port).Msg("Course Service gRPC server listening")
