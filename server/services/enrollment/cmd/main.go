...
func main() {
	...
	logger.Init(cfg.Env)
	log := logger.Get()
	log.Info().Msg("Starting Enrollment Service...")
	...
	repo := enrollment.NewPostgresRepository(pool)
	handler := enrollment.NewHandler(repo)
	...
	enrollmentpb.RegisterEnrollmentServiceServer(grpcServer, handler)
	reflection.Register(grpcServer)

	go func() {
		log.Info().Str("port", cfg.Port).Msg("Enrollment Service gRPC server listening")
